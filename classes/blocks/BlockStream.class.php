...
class BlockStream extends Block {
	...
	public function Exec() {
        ...
        if ($oTopic=$this->Topic_GetTopicsLast(Config::Get('block.stream.row'))) {
			$oViewers=$this->Viewer_GetLocalViewer();
            $oViewers->Assign('oTopics',$oTopic);
            $sTextResults=$oViewers->Fetch("blocks/block.stream_topic.tpl");
            $this->Viewer_Assign('sStreamTopic',$sTextResults);
        }
	}
}
?>