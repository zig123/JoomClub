...
var RLANG = {
	...
	outdent: 'Добавить тступ',
	indent: 'Убрать отступ',
	cancel: 'Отменить',
	insert: 'Вставить',
	save: 'Сохранить',
	_delete: 'Удалить',
	...
	rows: 'Строки',
	columns: 'Колонки',	
	add_head: 'Добавить оглавление',
	delete_head: 'Удалить оглавление',	
	title: 'Заголовок',
	...
	drop_file_here: 'Поставить файл сюда',
...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			airButtons: ['formatting', '|', 'bold', 'italic', 'deleted',  'underline', '|', 'unorderedlist', 'orderedlist', 'outdent', 'indent', '|', 'fontcolor', 'backcolor'],
			...
			modal_ls: String() + 
				'<form id="redactorInsertLsForm">' +
					'<label>' + RLANG.ls + '</label>' +
					'<input id="redactor_insert_ls_area" style="width: 99%; height: 20px;"></textarea>' +
				'</form>' +
				'<div id="redactor_modal_footer">' +
					'<span class="redactor_btns_box">' +
						'<input type="button" id="redactor_insert_ls_btn" value="' + RLANG.insert + '" />' +
	...
	Redactor.prototype = {
		...
		{
			this.saveSelection();
			this.modalInit(RLANG.ls, 'ls', 600, $.proxy(function()
				{
					$('#redactor_insert_ls_btn').click($.proxy(this.insertLs, this));
				}, this),
				
				function()
				{
					$('#redactor_insert_ls_area').focus();
				}
			);
		},
		insertLs: function()
		{
			var data = $('#redactor_insert_ls_area').val();
			data = this.stripTags(data);
			
			this.restoreSelection();
			this.execCommand('inserthtml', '<ls user="' +data+ '">&nbsp;');
			this.modalClose();
		},
