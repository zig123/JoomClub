...
<section id="comment_id_{$oComment->getId()}" class="comment
	...
	{if !$oComment->getDelete() or $bOneComment or ($oUserCurrent and $oUserCurrent->isAdministrator())}
		...
		<div class="comment_list_avatar"><a href="{$oUser->getUserWebPath()}"><img src="{$oUser->getProfileAvatarPath(48)}" alt="avatar" class="comment-avatar" /></a><br /><a href="{$oUser->getUserWebPath()}">{$oUser->getLogin()}</a></div>
		...
		<ul class="comment-info">
			...
			{if $oComment->getTargetType() != 'talk'}						
				<li id="vote_area_comment_{$oComment->getId()}" class="vote 
																		...
																		{if $oVote} 
																			...
																			{/if}
																		{/if}">
					<div class="vote-up icon-plus" onclick="return ls.vote.vote({$oComment->getId()},this,1,'comment');"></div>
					<span class="vote-count" id="vote_total_comment_{$oComment->getId()}">{if $oComment->getRating() > 0}+{/if}{$oComment->getRating()}</span>
					<div class="vote-down icon-minus" onclick="return ls.vote.vote({$oComment->getId()},this,-1,'comment');"></div>
				</li>
			{/if}
			...
			{if $oUserCurrent and !$bNoCommentFavourites}
				<li class="comment-favourite">
					<div onclick="return ls.favourite.toggle({$oComment->getId()},this,'comment');" class="favourite{if $oComment->getIsFavourite()} active{/if} icon-heart"></div>
