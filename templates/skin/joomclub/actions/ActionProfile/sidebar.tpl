...
<section class="block block-type-profile">
	...
	{if $sAction=='settings' and $oUserCurrent and $oUserCurrent->getId() == $oUserProfile->getId()}
		...
		<p class="upload-photo btn-group" style="margin-top: 10px">
			<a href="#" id="foto-upload" class="btn btn-primary">{if $oUserCurrent->getProfileFoto()}{$aLang.settings_profile_photo_change}{else}{$aLang.settings_profile_photo_upload}{/if}</a>&nbsp;&nbsp;&nbsp;
			<a href="#" id="foto-remove" class="btn btn-primary" onclick="return ls.user.removeFoto();" style="{if !$oUserCurrent->getProfileFoto()}display:none;{/if}">{$aLang.settings_profile_foto_delete}</a>
		</p>

		<div class="modal modal-upload-photo" id="foto-resize" style="z-index: 1050; display: none;">
