...
<form method="post" enctype="multipart/form-data" class="wrapper-content">
	...
	<input type="text" id="blog_title" name="blog_title" value="{$_aRequest.blog_title}" class="span12" />
	<small class="note">{$aLang.blog_create_title_notice}</small></p>

	
	<p><label for="blog_url">{$aLang.blog_create_url}:</label>
	<input type="text" id="blog_url" name="blog_url" value="{$_aRequest.blog_url}" class="span12" {if $_aRequest.blog_id and !$oUserCurrent->isAdministrator()}disabled{/if} />
	...
	<p><label for="blog_limit_rating_topic">{$aLang.blog_create_rating}<br /><small class="note">({$aLang.blog_create_rating_notice})</small>:</label>
	...
	{else}
	<textarea name="blog_description" id="redactor" rows="15" class="input-text input-width-full input-width-full">{$_aRequest.blog_description}</textarea>
	{/if}
