...
<div class="tabbable">
      <ul class="nav nav-tabs">
        <li class="active"><a data-toggle="tab" href="#{$oUserProfile->getLogin()}1">{$aLang.profile_about}</a></li>
        <li><a data-toggle="tab" href="#{$oUserProfile->getLogin()}2">{$aLang.profile_privat}</a></li>
        <li><a data-toggle="tab" href="#{$oUserProfile->getLogin()}3">{$aLang.profile_activity}</a></li>
        <li><a data-toggle="tab" href="#{$oUserProfile->getLogin()}4">{$aLang.profile_friends}</a></li>
        <li><a data-toggle="tab" href="#{$oUserProfile->getLogin()}5">{$aLang.profile_contacts}</a></li>
      </ul>
      <div class="tab-content">
        ...
        <div id="{$oUserProfile->getLogin()}3" class="tab-pane">
		  <ul class="unstyled">
			...
			{/if}
			{if $aBlogsOwner}
				<li><span>{$aLang.profile_blogs_self}:</span><strong>{foreach from=$aBlogsOwner item=oBlog name=blog_owner}<a href="{$oBlog->getUrlFull()}">{$oBlog->getTitle()|escape:'html'}</a>{if !$smarty.foreach.blog_owner.last}, {/if}{/foreach}</strong></li>
			{/if}
        ...
        <div id="{$oUserProfile->getLogin()}5" class="tab-pane">
    ...
    {if $oUserCurrent and $oUserCurrent->getId() == $oUserProfile->getId()}<br /><br /><a href="{router page='settings'}" class="edit btn btn-primary">{$aLang.profile_about_edit}</a>{/if}
