...
<h5><a href="#" onclick="ls.ad.toggle(0); return false;">{$aLang.adunits_menu_settings_add_title}</a></h5>
...
{*	<option value="bl"{if $_aRequest.adunits_setting_viev_1 == 'bl'} selected{/if}>{$aLang.adunits_menu_settings_viev_bl}</option>
	...
	<p class="buttons" align="right">
	    ...
	    <a href="#" onclick="ls.ad.toggle(0); return false;" style="color: red; clear: both;">{$aLang.adunits_fohm_hide}</a>
	</p>
...
{if count($aAdunits)}
	...
	<div id="au_{$oAdunits->getAdunitsId()}">
	    ...
	    <a href="#" onclick="ls.ad.toggle({$oAdunits->getAdunitsId()}); return false;"><img src="{$sTemplateWebPathPlugin}img/edit.png" alt="{$aLang.adunits_menu_settings_edit_title}" title="{$aLang.adunits_menu_settings_edit_title}" /> {$aLang.adunits_menu_settings_edit_title}</a>
	    <a href="#" onclick="ls.ad.del('{$oAdunits->getAdunitsId()|escape:'html'}'); return false;" style="text-decoration: none; color: red;"><img src="{$sTemplateWebPathPlugin}img/delete.png" alt="{$aLang.adunits_menu_settings_delete}" title="{$aLang.adunits_menu_settings_delete}" /> {$aLang.adunits_menu_settings_dellete_title}</a>
...
{*		    <option value="b"{if $oAdunits->getAdunitsSetting() == 'b'} selected{/if}>{$aLang.adunits_menu_settings_viev_b}</option>
		    ...
		    <input type="submit" name="submit_adunits_save" value="{$aLang.adunits_submit_save}" onclick="ls.ad.save('{$oAdunits->getAdunitsId()}');" /> <br />
		    <a href="#" onclick="ls.ad.toggle({$oAdunits->getAdunitsId()}); return false;" style="color: red;">{$aLang.adunits_fohm_hide}</a>
		</form>
		<br /><br />
	    </div>
	</div>
