{foreach from=$aReplyWall item=oReplyWall}
	...
	<div id="wall-reply-item-{$oReplyWall->getId()}" class="js-wall-reply-item wall-item-reply span12">
		...
		<ul class="actions">
			...
			{if $oReplyWall->isAllowDelete()}
				<li><a href="#" onclick="return ls.wall.remove({$oReplyWall->getId()});" class="btn btn-primary">{$aLang.wall_action_delete}</a></li>
			{/if}
		</ul>
	</div>
{/foreach}